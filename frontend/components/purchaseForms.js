...
export default function PurchaseForm({
  ...
  useEffect(() => {
    ...
    if (form.classification === "taxable") {
      const calculatedVat = (amountNumber * 0.1).toFixed(2);

      if (form.vatAmount !== calculatedVat) {
        onChange("vatAmount", calculatedVat);
      }
    } else {
      if (form.vatAmount !== "0.00") {
        onChange("vatAmount", "0.00");
      }
    }
  }, [form.amount, form.classification, form.vatAmount, onChange]);
    ...
    <View style={styles.formCard}>
      <View style={styles.fieldGroup}>
        ...
        <TextInput
          ...
          onChangeText={(value) => onChange("amountExVat", value)}
