...
const buildFilingPackData = async (filingId) => {
  ...
  const transactionResult = await pool.query(
    ...
    `,
    [filing.company_id, filing.start_date, filing.end_date]
  );
...
exports.saveVatFiling = async (req, res) => {
  try {
    ...
    if (!companyId || !start_date || !end_date) {
      return res.status(400).json({
        error: "companyId, start_date, and end_date are required",
      });
    }
      ...
      rows.forEach((row, rowIndex) => {
        ...
        columns.forEach((col) => {
          doc
            ...
            .fontSize(9)
            .fillColor("#111827")
            .text(String(row[col.key] ?? "-"), cellX + 6, y + 5, {
              width: col.width - 12,
              align: col.align || "left",
              ellipsis: true,
            });
