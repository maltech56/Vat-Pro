...
const {
  buildFilingSnapshot
} = require("../services/filingSnapshotService");
...
const buildFilingPackData = async (filingId) => {
  ...
  const transactions = filing.supporting_transactions || [];

  const documents = filing.linked_documents || [];

  const transactionsMissingDocuments =
    filing.missing_document_warnings || [];

  const transactionCount = Number(filing.transaction_count || 0);

  const linkedTransactionCount = Number(
    filing.linked_transaction_count || 0
  );

  const missingDocumentCount = Number(
    filing.missing_document_count || 0
  );
  ...
  const auditScore = Number(filing.audit_score || 0);

  const auditReadiness =
    filing.audit_readiness || "unknown";
...
// ============================================
// Read immutable audit snapshot from VAT filing
// ============================================

const getStoredAuditSnapshot = async (filingId) => {
  ...
  return {
    auditScore: Number(filing.audit_score || 0),
    auditReadiness: filing.audit_readiness || "unknown",
    transactionCount: Number(filing.transaction_count || 0),
...
exports.saveVatFiling = async (req, res) => {
  try {
    ...
    const filingData = {
      ...
      outputVat: numericOutputVat,
      inputVat: numericInputVat,
      netVat: numericNetVat,
    ...
    //----------------------------------------------------------
    // Build immutable filing snapshot
    //----------------------------------------------------------

    const snapshot = await buildFilingSnapshot({
      companyId: filingData.companyId,

      periodStart: filingData.startDate,

      periodEnd: filingData.endDate,
  ...
  `,
      [
        filingData.companyId,
        filingData.startDate,
        filingData.endDate,
      ]
    );
    ...
    if (existing.rows.length > 0) {
      ...
      const currentStatus = String(existingFiling.status || "").toLowerCase();

      if (currentStatus === "submitted") {
        return res.status(400).json({
          error: "Submitted filings cannot be modified.",
        });
      }

      if (currentStatus === "locked") {
        return res.status(400).json({
          error: "Locked filings cannot be modified.",
        });
      }

      // Existing draft → UPDATE instead of INSERT
      const updateResult = await pool.query(
        `
UPDATE vat_filings
SET
    tin = $2,
    filing_period_label = $3,
    ...
    authorized_officer = $14,
    position_title = $15,
    declaration_accepted = $16,

    audit_score = $17,
    audit_readiness = $18,

    transaction_count = $19,
    linked_transaction_count = $20,
    missing_document_count = $21,
    document_count = $22,

    supporting_transactions = $23,
    linked_documents = $24,
    missing_document_warnings = $25,
...
`,
        [
          ...
          filingData.tin,
          filingData.filingPeriodLabel,

          filingData.startDate,
          filingData.endDate,

          filingData.taxableSales,
          filingData.zeroRatedSales,
          filingData.exemptSales,

          filingData.totalSales,
          filingData.totalPurchases,

          filingData.outputVat,
          filingData.inputVat,
          filingData.netVat,

          filingData.authorizedOfficer,
          filingData.positionTitle,
          filingData.declarationAccepted,

          snapshotAudit.auditScore,
          snapshotAudit.auditReadiness,

          snapshotStats.transactionCount,
          snapshotStats.linkedTransactionCount,
          snapshotStats.missingDocumentCount,
          snapshotStats.documentCount,

          JSON.stringify(snapshot.transactions),
          JSON.stringify(snapshot.linkedDocuments),
      ...
      return res.json({
        ...
        filing: mapFilingRow(updateResult.rows[0]),
      });
    }
...
exports.updateFilingStatus = async (req, res) => {
  ...
  `,
    ...
    if (status === "submitted") {
      const snapshot = await getStoredAuditSnapshot(filingId);

      const auditScore = snapshot.auditScore;

      const missingDocumentCount =
        snapshot.missingDocumentCount;
